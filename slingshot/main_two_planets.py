...
from config import second_planet
...
# second planet
second_planet_name = planet_names[second_planet-1]
second_planet_values = planets[second_planet_name]
SECOND_PLANET_MASS = second_planet_values["mass"]
SECOND_G = second_planet_values["gravity"]
SECOND_PLANET_SIZE = second_planet_values["planet_size"]
...
second_planet_images = {
    1: pygame.transform.scale(pygame.image.load('images/1.png'), (SECOND_PLANET_SIZE * 2, SECOND_PLANET_SIZE * 2)),
    2: pygame.transform.scale(pygame.image.load('images/2.png'), (SECOND_PLANET_SIZE * 2, SECOND_PLANET_SIZE * 2)),
    3: pygame.transform.scale(pygame.image.load('images/3.png'), (SECOND_PLANET_SIZE * 2, SECOND_PLANET_SIZE * 2)),
    4: pygame.transform.scale(pygame.image.load('images/4.png'), (SECOND_PLANET_SIZE * 2, SECOND_PLANET_SIZE * 2))
}

BG = pygame.transform.scale(pygame.image.load('images/background.jpg'), (WIDTH, HEIGHT))
PLANET = planet_images[chosen_planet]
SECOND_PLANET = second_planet_images[second_planet]
...
class Planet:
    ...
    def draw(self):
        ...
        second_planet_rect = SECOND_PLANET.get_rect(center=(self.x2, self.y2))
        win.blit(SECOND_PLANET, second_planet_rect.topleft)
...
class Spacecraft2:
    ...
    def move(self, planet1=None, planet2=None):
        # Reset acceleration
        acceleration_x = 0
        acceleration_y = 0
        ...
        if planet1 and planet2:
            ...
            # Calculate force and angle for planet 2
            distance_to_planet2 = math.sqrt((self.x - planet2.x2) ** 2 + (self.y - planet2.y2) ** 2)
            force_from_planet2 = (SECOND_G * self.mass * planet2.mass) / distance_to_planet2 ** 2
            angle_to_planet2 = math.atan2(planet2.y2 - self.y, planet2.x2 - self.x)

            # Calculate total force and acceleration
            total_force_x = (force_from_planet1 * math.cos(angle_to_planet1)) + (force_from_planet2 * math.cos(angle_to_planet2))
            total_force_y = (force_from_planet1 * math.sin(angle_to_planet1)) + (force_from_planet2 * math.sin(angle_to_planet2))
...
def create_ship(location, mouse):
    ...
    obj = Spacecraft2(t_x, t_y, vel_x, vel_y, SHIP_MASS)
    return obj
...
def main():
    ...
    planet2 = Planet(WIDTH // 2, HEIGHT // 2, SECOND_PLANET_MASS)
    objects = []
    temp_obj_pos = None
    ...
    while running:
        ...
        for obj in objects[:]:
            obj.draw()
            obj.move(planet1, planet2)  # Pass both planet and second_planet instances
            off_screen = obj.x < 0 or obj.x > WIDTH or obj.y < 0 or obj.y > HEIGHT
            collided = math.sqrt((obj.x - planet1.x) ** 2 + (obj.y - planet2.y) ** 2) <= PLANET_SIZE
            collided_with_second = math.sqrt((obj.x - planet2.x2) ** 2 + (obj.y - planet2.y2) ** 2) <= SECOND_PLANET_SIZE

            if off_screen or collided or collided_with_second:
                objects.remove(obj)
